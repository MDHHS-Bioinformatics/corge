...
| `--hc_thresholds`     | string | ✓        | `15,20,40,150`    | Hierarchical-clustering thresholds (distance) for grouping samples. Comma-separated (no spaces). |
...
| `--reference_path`     | string  | –       | Path to reference/representative assembly FASTA used to generate the Prodigal training file for `--mode create_schema`. |
